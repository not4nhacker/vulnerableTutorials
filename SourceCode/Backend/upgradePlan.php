...
<?php
    /*Questa pagina inserisce il nuovo piano dentro alla tabella e avvisa l'utente se tutto è andato a buon fine*/
    ...
    * Inizialmente andiamo ad unire username e password per poi verificare che il loro hash corrisponda al cookie di sessione*/
    ...
    /*Prende l'id Del piano desiderato e la data corrente*/
    $idPlan = $_GET["idPlan"];
    $date = date("Y-m-d");


    $insertAcquisition = "INSERT INTO acquisition (userId, idPlan, acquisitionDate) VALUES ($userId, $idPlan, \"$date\");";
    if(!$conn->query($insertAcquisition)) {
        die("Errore durante la query " . $conn->error);
    } else{
        echo "<div id='container'>
        <section id='greetings'>
        <h2>Piano acquistato con successo</h2>
        <p>Hai effettuato l'upgrade al tuo nuovo piano con successo, clicca qui sotto per tornare al tuo profilo e iniziare un nuovo corso</p>
